...
class Trainer:
    ...
    def validation(self, i_iter):
        ...
        if not self.args.val_only:
            print(f'Current loss at iter {i_iter}: {new_loss}')

            # Check if this is the new best model within the current window and save state it it is
            if not self.val_losses or new_loss < min(self.val_losses, key=lambda x: x[0])[0]:
                print(f'saving the best model within the current window at iter {i_iter} with loss {new_loss}')
                self.best_model_state = copy.deepcopy(self.model.state_dict())
                self.best_model_D1_state = copy.deepcopy(self.model_D1.state_dict())
                self.best_model_D2_state = copy.deepcopy(self.model_D2.state_dict())
                self.best_model_wD_state = copy.deepcopy(self.model_wD.state_dict())
                self.best_model_info = (i_iter, new_loss)

            self.val_losses.append((new_loss, i_iter))
            # print('val_losses: ', self.val_losses)

            if len(self.val_losses) == 10:
                avg_loss = np.mean([loss for loss, _ in self.val_losses])
                if self.last_avg_loss is not None and avg_loss > self.last_avg_loss:
                    ...
                    if self.patience_count >= self.args.num_steps_stop:
                        print(f"Early stopping at iteration {i_iter}. No improvement in average loss for {self.args.num_steps_stop} evaluations.")
                        if self.logger_fid:
                            print(f"Early stopping at iteration {i_iter}. No improvement in average loss for {self.args.num_steps_stop} evaluations.",
                    ...
                    self.patience_count = 0
                    # Find and save the model with the lowest loss in the current window
                    self.save_model(self.best_model_info[0], self.best_model_info[1])

                self.last_avg_loss = avg_loss
                print('avg_loss: ', self.last_avg_loss)
                self.val_losses = []

            return False


    def save_model(self, i_iter, new_loss):
        print(f"Storing new best model at iteration {i_iter} with loss {new_loss}")
        if self.logger_fid:
            print(f"Storing new best model at iteration {i_iter} with loss {new_loss}", file=self.logger_fid)
        torch.save(self.best_model_state, osp.join(self.args.snapshot_dir, f'G-{self.args.dataset_source}-{self.args.dataset_target}.pth'))
        torch.save(self.best_model_D1_state, osp.join(self.args.snapshot_dir, f'D1-{self.args.dataset_source}-{self.args.dataset_target}.pth'))
        torch.save(self.best_model_D2_state, osp.join(self.args.snapshot_dir, f'D2-{self.args.dataset_source}-{self.args.dataset_target}.pth'))
        torch.save(self.best_model_wD_state, osp.join(self.args.snapshot_dir, f'wD-{self.args.dataset_source}-{self.args.dataset_target}.pth'))
